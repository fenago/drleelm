...
export default function Sidebar() {
  ...
  const chatButtonClass = `p-2 rounded-xl duration-300 transition-all ${isOpen ? 'text-white bg-stone-800' : 'hover:bg-stone-900 hover:text-stone-200'}`

  return (
    <aside ref={sidebarRef} className='left-0 bottom-0 w-screen md:w-fit md:h-screen fixed p-4 z-50 lg:flex'>
      ...
      <div className={`w-64 md:order-2 h-full z-40 rounded-l-none rounded-2xl bg-stone-950 border border-l-transparent border-stone-900 flex-col p-4 space-y-3 overflow-y-auto custom-scroll transition-all duration-300 ${isOpen ? 'flex' : 'hidden'}`}>
      ...
      <div className={`md:w-20 md:order-1 h-full rounded-3xl bg-stone-950/50 backdrop-blur-xl border border-stone-900 text-stone-400 flex flex-col items-center justify-between py-4 ${isOpen ? 'md:rounded-r-none' : ''}`}>
        ...
        <nav className='flex md:flex-col items-center space-x-4 md:space-x-0 md:space-y-2 my-auto'>
          ...
          <button onClick={() => setIsOpen(!isOpen)} className={chatButtonClass}>
            <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><path fillRule="evenodd" clipRule="evenodd" d="M12 1.25C6.06294 1.25 1.25 6.06294 1.25 12C1.25 17.9371 6.06294 22.75 12 22.75C17.9371 22.75 22.75 17.9371 22.75 12C22.75 6.06294 17.9371 1.25 12 1.25ZM12.75 8C12.75 7.58579 12.4142 7.25 12 7.25C11.5858 7.25 11.25 7.58579 11.25 8L11.25 11.5C11.25 12.7426 12.2574 13.75 13.5 13.75H15C15.4142 13.75 15.75 13.4142 15.75 13C15.75 12.5858 15.4142 12.25 15 12.25H13.5C13.0858 12.25 12.75 11.9142 12.75 11.5L12.75 8Z" fill="currentColor" /></svg>
          </button>
